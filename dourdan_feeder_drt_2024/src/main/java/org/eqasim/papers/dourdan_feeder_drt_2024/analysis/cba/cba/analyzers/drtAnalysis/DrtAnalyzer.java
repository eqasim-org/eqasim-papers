...
import org.eqasim.papers.dourdan_feeder_drt_2024.analysis.cba.cba.analyzers.CbaAnalyzer;
    ...
    public void handleEvent(PassengerRequestScheduledEvent event) {
        if (event.getMode().equals(mode)) {
            for(Id<Person> personId: event.getPersonIds()) {
                this.personsRequests.put(personId, event.getRequestId());
                performedRequestSequences.put(event.getRequestId(),
                        new PerformedRequestEventSequence(requestSubmissions.get(event.getRequestId()), event, this.personsLastFinishedActivity.get(personId), this.lastDeparturePerPerson.get(personId).getRoutingMode()));
            }
        }
    }
